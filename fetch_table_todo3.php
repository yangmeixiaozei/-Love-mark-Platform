...
<div>
	<table class="table-style2" width="96%" border="1">
		...
		<tbody>
			...
			$result = mysqli_query($conn, "SELECT * FROM todo where phone = '{$_SESSION['phone_number']}' and rank = '不重要但紧急' and state = '未开始' ORDER BY time ASC");
			$count = 0;
			while ($row = mysqli_fetch_array($result)) {
				if ($count % 2 == 0){
					...
					<tr class="tr_line2">
						...
						<td style="width: 150px;"><a href="todo_start.php?id=<?php echo $row[ 'ID']; ?> ">开始</a></td>
					</tr>
						<?php
				}
				else{
